...
class Solution {
    static ArrayList<ArrayList<Integer>> nQueen(int n) {
        ...
        for(int i = 0; i < n; i++) {    // traverse for each cell of the first row
            ...
            nQueenHelper(board, canPlaceQueen, list, permutation, 0, i); // call to actual shit
        }
        return list;
    }

    static void nQueenHelper(boolean[][] board, boolean[][] canPlaceQueen, ArrayList<ArrayList<Integer>> list, ArrayList<Integer> permutation, int row, int col) {
        if (canPlaceQueen[0][col] || canPlaceQueen[1][(board.length - 1) + col-row] || canPlaceQueen[2][col+row]) return;   // return if another queen can attack this cell
        ...
        canPlaceQueen[0][col] = canPlaceQueen[1][(board.length - 1) + col-row] = canPlaceQueen[2][col+row] = true; // mark the column and diagonals as occupied
        
        for(int i = 0; i < board.length && ((row+1) < board.length); i++) { // find all possible safe permutations for the next queen in the next row
            nQueenHelper(board, canPlaceQueen, list, permutation, row+1, i);
        }
        ...
        board[row][col] = false;
        canPlaceQueen[0][col] = canPlaceQueen[1][(board.length - 1) + col-row] = canPlaceQueen[2][col+row] = false;
    }
}
